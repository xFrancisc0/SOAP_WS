<?php
    ...
    $url = "https://www.learnwebservices.com/services/hello?WSDL";
    $client = new SoapClient($url);

    print_r($client->__getFunctions());
    print_r($client->__getTypes());

    //El output de ambos print_r es:
    /*
    Array
        (
    ...
    Es decir, la estructura sería  SayHello(HelloRequest)  y HelloRequest es un objeto que tiene un atributo llamado Name
    Entonces lo que se le debería enviar es SayHello(HelloRequest->Name)
    */

    class StructSayHello {
        public $Name;
    }

    $Input = new StructSayHello;
    $Input->Name = "Francisco";

    var_dump($client->SayHello(["HelloRequest" => $Input])->HelloResponse->Message );
